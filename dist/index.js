...
const argv = await yargs(hideBin(process.argv))
    ...
    type: "string",
    description: "Path to a YAML file defining custom clause patterns (see custom-clause-patterns.example.yaml)",
})
    .option("bulk-concurrency", {
    type: "number",
    description: "Number of files to process in parallel during bulk_analyze (default: 4)",
    default: 4,
})
    .option("ocr", {
    type: "boolean",
    description: "Enable OCR fallback for scanned PDFs that contain no text layer. Requires optional dependencies: tesseract.js, pdfjs-dist, and canvas",
    default: false,
})
    .option("http-port", {
    type: "number",
    description: "Start the server in Streamable HTTP mode on the given port instead of stdio",
})
    .strict()
    .help()
    .parseAsync();
