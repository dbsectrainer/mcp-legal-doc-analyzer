...
import { CallToolRequestSchema, CancelledNotificationSchema, ErrorCode, GetPromptRequestSchema, ListPromptsRequestSchema, ListResourcesRequestSchema, ListToolsRequestSchema, McpError, ReadResourceRequestSchema, } from "@modelcontextprotocol/sdk/types.js";
import { readFileSync } from "fs";
import { createDb } from "./db.js";
import { loadAllTemplates } from "./templates.js";
import { toolExtractClauses, toolFlagRisks, toolSummarizeTerms, } from "./tools/analyze.js";
...
function registerCancellationHandler(server) {
...
export function createServer(config) {
    ...
    function emitLowConfidenceWarning(filePath, clauseType, confidence) {
        emitLog("info", `Low-confidence extraction in "${filePath}": clause type "${clauseType}" scored ${Math.round(confidence * 100)}% — human review recommended.`);
    }
    // ---------------------------------------------------------------------------
    // List tools
    // ---------------------------------------------------------------------------
            ...
            tools: [
                {
                    ...
                    description: "Extract and label key clauses from a contract document (PDF, DOCX, or TXT). Returns a structured list of clause types (e.g. termination, liability, confidentiality, governing_law, payment_terms, intellectual_property, dispute_resolution, warranty) with confidence scores. Low-confidence results are flagged for human review.",
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            file_path: {
                                type: "string",
                                description: "Absolute path (or ~/... path) to the contract file. Supported formats: .pdf, .docx, .txt",
                            },
                        },
                        required: ["file_path"],
                ...
                {
                    name: "flag_risks",
                    description: "Analyze a contract against a compliance template and return a severity-ranked list of risk findings. Available templates: NDA, Employment Agreement, SaaS Customer Agreement, Software License Agreement, Vendor Agreement, Consulting Agreement. If no template is specified, the NDA template is used.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {
                            ...
                            template: {
                                type: "string",
                                description: "Name of the compliance template to use. Examples: NDA, Employment Agreement, SaaS Customer Agreement, Software License Agreement, Vendor Agreement, Consulting Agreement. Use list_templates to see all available options.",
                            },
                        },
                        required: ["file_path"],
                        additionalProperties: false,
                ...
                },
                {
                    ...
                    description: "Generate a plain-English, bullet-point summary of the key provisions in a contract document. Covers clause types including termination, liability, confidentiality, governing_law, payment_terms, intellectual_property, dispute_resolution, and warranty.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {
                ...
                {
                    name: "list_templates",
                    description: "List all available compliance templates that can be used with flag_risks, check_compliance, and export_analysis_report. Bundled templates include: NDA, Employment Agreement, SaaS Customer Agreement, Software License Agreement, Vendor Agreement, Consulting Agreement.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {},
                ...
                },
                {
                    ...
                    description: "Run a structured checklist validation of a contract against a compliance template. For each rule, returns pass/fail/not_applicable with a finding description. Available templates: NDA, Employment Agreement, SaaS Customer Agreement, Software License Agreement, Vendor Agreement, Consulting Agreement.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {
                            ...
                            template: {
                                type: "string",
                                description: "Name of the compliance template to use. Examples: NDA, Employment Agreement, SaaS Customer Agreement, Software License Agreement, Vendor Agreement, Consulting Agreement.",
                            },
                        },
                        required: ["file_path"],
                        additionalProperties: false,
                ...
                },
                {
                    ...
                    description: "Compare two versions of a contract document at the clause level. Shows clauses that were added, removed, changed, or unchanged between version A and version B. Supports .pdf, .docx, and .txt files.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {
                            file_path_a: {
                                type: "string",
                                description: "Absolute path (or ~/... path) to version A of the contract. Supported formats: .pdf, .docx, .txt",
                            },
                            file_path_b: {
                                type: "string",
                                description: "Absolute path (or ~/... path) to version B of the contract. Supported formats: .pdf, .docx, .txt",
                            },
                        },
                ...
                },
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            ...
                            template: {
                                type: "string",
                                description: "Optional compliance template name to include risk findings in the report. Examples: NDA, Employment Agreement, SaaS Customer Agreement, Software License Agreement, Vendor Agreement, Consulting Agreement.",
                            },
                        },
                        required: ["file_path"],
                        additionalProperties: false,
                ...
                },
                {
                    ...
                    description: "Export the analysis audit log. Supports optional date-range filtering and JSON or CSV output format.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {
                ...
                {
                    name: "bulk_analyze",
                    description: "Analyze a portfolio of contract files in batch. Processes each file sequentially and returns per-file clause extraction results plus an aggregate summary.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            file_paths: {
                            ...
                            template_name: {
                                type: "string",
                                description: "Compliance template to apply. Examples: NDA, Employment Agreement, SaaS Customer Agreement.",
                            },
                            output_format: {
                                type: "string",
                                enum: ["json", "csv"],
        ...
        try {
            switch (name) {
                case "extract_clauses": {
                    ...
                    const onProgress = (step, total, _desc) => {
                        server
                            .notification({
                    ...
                    const extractResult = await toolExtractClauses(filePath, resolvedConfig, db, emitLowConfidenceWarning, onProgress);
                ...
                case "flag_risks": {
                    ...
                    const riskTemplate = typeof safeArgs["template"] === "string" ? safeArgs["template"] : undefined;
                    emitLog("info", `Document loaded for risk analysis: ${filePath}`);
                    const riskResult = await toolFlagRisks(filePath, riskTemplate, templates, resolvedConfig, db);
                    // Audit log (best-effort)
                    try {
                        const resolvedPath = expandHome(filePath);
                        const rawBytes = readFileSync(resolvedPath);
                ...
                case "check_compliance": {
                    ...
                    const complianceTemplate = typeof safeArgs["template"] === "string" ? safeArgs["template"] : undefined;
                    emitLog("info", `Document loaded for compliance check: ${filePath}`);
                    const complianceResult = await toolCheckCompliance(filePath, complianceTemplate, templates, resolvedConfig, db);
                    // Audit log (best-effort)
                    try {
                        const resolvedPath = expandHome(filePath);
                        const rawBytes = readFileSync(resolvedPath);
                ...
                case "compare_versions": {
                    ...
                    if (typeof filePathA !== "string" || !filePathA.trim()) {
                        throw new McpError(ErrorCode.InvalidParams, "file_path_a must be a non-empty string");
                    }
                    if (typeof filePathB !== "string" || !filePathB.trim()) {
                        throw new McpError(ErrorCode.InvalidParams, "file_path_b must be a non-empty string");
                    }
                    emitLog("info", `Document comparison started: ${filePathA} vs ${filePathB}`);
                    const compareResult = await toolCompareVersions(filePathA, filePathB, resolvedConfig);
                ...
                case "export_analysis_report": {
                    ...
                    const reportTemplate = typeof safeArgs["template"] === "string" ? safeArgs["template"] : undefined;
                    emitLog("info", `Export started: ${filePath}`);
                    const reportResult = await toolExportAnalysisReport(filePath, reportTemplate, templates, resolvedConfig);
                    emitLog("info", `Export complete: ${filePath}`);
                    return { content: [{ type: "text", text: reportResult }] };
                }
                case "export_audit_log": {
                    const fromDate = typeof safeArgs["from_date"] === "string" ? safeArgs["from_date"] : undefined;
                    const toDate = typeof safeArgs["to_date"] === "string" ? safeArgs["to_date"] : undefined;
                    const format = typeof safeArgs["format"] === "string" && safeArgs["format"] === "csv"
                        ? "csv"
                        : "json";
                    const entries = auditLog.export(fromDate, toDate);
                    let output;
                    if (format === "csv") {
                        const header = "timestamp,doc_fingerprint,template_used,tool_name,requester_id,clause_count";
                ...
                case "bulk_analyze": {
                    ...
                    if (!Array.isArray(filePaths) || filePaths.length === 0) {
                        throw new McpError(ErrorCode.InvalidParams, "file_paths must be a non-empty array of strings");
                    }
                    const templateName = safeArgs["template_name"];
                    if (typeof templateName !== "string" || !templateName.trim()) {
                        throw new McpError(ErrorCode.InvalidParams, "template_name must be a non-empty string");
                    }
                    const outputFormat = typeof safeArgs["output_format"] === "string" &&
                    ...
                    const bulkResult = await bulkAnalyze(rawDb, filePaths, templateName, {
                    ...
                    }, templates, (index, total, filePath, _result) => {
                        server
                            .notification({
                            method: "notifications/progress",
                    ...
                    if (outputFormat === "csv") {
                        ...
                        const rows = bulkResult.results.map((r) => [r.filePath, String(r.success), String(r.clauseCount), r.error ?? ""]
                            .map((v) => `"${v.replace(/"/g, '""')}"`)
                            .join(","));
            ...
            if (typed.mcpCode === "invalid_params") {
                throw new McpError(ErrorCode.InvalidParams, typed.message ?? "Invalid parameters");
            }
            throw new McpError(ErrorCode.InternalError, typed.message ?? "Internal server error");
        }
    });
    // ---------------------------------------------------------------------------
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
        ...
        if (!db) {
            throw new McpError(ErrorCode.InternalError, "History is disabled (--no-history). Resources are not available.");
        }
        const analysis = db.getAnalysis(analysisId);
        if (!analysis) {
            throw new McpError(ErrorCode.InvalidParams, `No analysis found with id: ${analysisId}`);
        }
        const lines = [
            ...
            prompts: [
                {
                    ...
                    description: "Guided structured contract review workflow. Walks through clause extraction, risk flagging, compliance checking, and plain-English summarisation for a given document.",
                    arguments: [
                        {
                            name: "file_path",
                            description: "Absolute path (or ~/... path) to the contract file to review.",
                            required: true,
                        },
                        {
                            name: "template",
                            description: "Compliance template to apply (e.g. NDA, Employment Agreement, SaaS Customer Agreement, Software License Agreement, Vendor Agreement, Consulting Agreement). Defaults to NDA.",
                            required: false,
                        },
                    ],
                },
    ...
    server.setRequestHandler(GetPromptRequestSchema, async (request) => {
        ...
        const filePath = promptArgs && typeof promptArgs["file_path"] === "string"
            ? promptArgs["file_path"]
            : "<file_path>";
        const template = promptArgs && typeof promptArgs["template"] === "string"
            ? promptArgs["template"]
            : "NDA";
        return {
