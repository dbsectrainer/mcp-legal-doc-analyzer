 ...
 */
export async function toolExtractClauses(filePath, config, db, onLowConfidence, onProgress) {
    const TOTAL_STEPS = 3;
    if (onProgress)
        onProgress(1, TOTAL_STEPS, "Parsing document");
    let doc;
    try {
    ...
    if (onProgress)
        onProgress(2, TOTAL_STEPS, "Extracting clauses");
    const clauses = extractClauses(doc.text, config.confidenceThreshold, config.clausePatternsFile);
    if (onProgress)
        onProgress(3, TOTAL_STEPS, "Finalising results");
    ...
    if (doc.ocr_used) {
        lines.push("> ⚠️ **OCR fallback used**: No text layer was detected in this PDF. Text was extracted via OCR — accuracy may be lower than for text-based PDFs. Human review is strongly recommended.");
        lines.push("");
    }
    if (clauses.length === 0) {
        ...
        for (const clause of clauses) {
            ...
            const lowFlag = clause.low_confidence
                ...
                : "";
            lines.push(`### ${formatClauseType(clause.type)} (${confidencePct}% confidence)${lowFlag}`);
            lines.push("");
            // Truncate very long clause text in the output to keep responses readable
            const displayText = clause.text.length > 500 ? clause.text.slice(0, 500) + "…" : clause.text;
...
export async function toolFlagRisks(filePath, templateName, templates, config, db) {
    ...
    if (!template) {
        throw Object.assign(new Error(`Template "${resolvedTemplateName}" not found. Use list_templates to see available templates.`), { mcpCode: "invalid_params" });
    }
    let doc;
    try {
        doc = await parseDocument(filePath, { enableOcr: config.enableOcr });
    ...
    if (findings.length === 0) {
        ...
        for (const finding of findings) {
            ...
            lines.push(`**Type:** ${finding.flag_type === "missing_clause" ? "Missing clause" : "Pattern match"}`);
...
export async function toolSummarizeTerms(filePath, config) {
    ...
    if (clauses.length === 0) {
        lines.push("- No identifiable key provisions were found above the confidence threshold.");
    }
    else {
