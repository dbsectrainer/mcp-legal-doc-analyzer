...
async function extractPdfText(buffer) {
    try {
        ...
        throw Object.assign(new Error(`Failed to parse PDF: ${err instanceof Error ? err.message : String(err)}`), { code: "PDF_PARSE_ERROR" });
    }
}
/**
...
async function extractPdfTextWithOcr(buffer) {
    ...
    try {
        ...
        throw Object.assign(new Error("OCR fallback requires tesseract.js. Install it with: npm install tesseract.js"), { code: "OCR_DEPENDENCY_MISSING" });
    }
    ...
    try {
        ...
        throw Object.assign(new Error("PDF OCR requires pdfjs-dist and canvas. Install with: npm install pdfjs-dist canvas"), { code: "OCR_DEPENDENCY_MISSING" });
    }
...
async function extractDocxText(buffer) {
    try {
        ...
        throw Object.assign(new Error(`Failed to parse DOCX: ${err instanceof Error ? err.message : String(err)}`), { code: "DOCX_PARSE_ERROR" });
    }
}
export async function parseDocument(filePath, options) {
    ...
    try {
        ...
        throw Object.assign(new Error(`Cannot read file at path "${resolvedPath}": ${err instanceof Error ? err.message : String(err)}`), { code: "FILE_READ_ERROR" });
    }
