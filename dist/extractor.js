 ...
 */
export function loadCustomClausePatterns(filePath) {
    ...
    try {
        ...
        for (const entry of raw) {
            if (typeof entry.name !== "string" || !Array.isArray(entry.patterns))
                continue;
            result[entry.name] = {
                patterns: entry.patterns.map((p) => new RegExp(p, "i")),
                baseConfidence: typeof entry.base_confidence === "number" ? entry.base_confidence : 0.75,
 ...
 */
export function buildClausePatterns(customPatternsFile) {
...
// ── Confidence scoring ────────────────────────────────────────────────────────
function calculateConfidence(text, patterns, baseConfidence) {
    let matchCount = 0;
 ...
 */
export function extractClauses(text, confidenceThreshold = 0.6, customPatternsFile) {
    ...
    for (const section of sections) {
        ...
        for (const [clauseType, config] of Object.entries(patterns)) {
            const confidence = calculateConfidence(section.text, config.patterns, config.baseConfidence);
