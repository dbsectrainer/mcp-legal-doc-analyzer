...
export async function bulkAnalyze(
  ...
  filePaths: string[],
  templateName: string,
  options: { confidenceThreshold?: number; concurrency?: number; enableOcr?: boolean },
  templates: Map<string, Template>,
  onProgress?: ProgressCallback,
): Promise<BulkAnalysisResult> {
  const template: Template | undefined = getTemplate(templates, templateName);
  if (!template) {
    throw Object.assign(
      new Error(
  ...
  const results: BulkResult[] = new Array(filePaths.length);
  let completed = 0;

  async function processFile(filePath: string, index: number): Promise<void> {
