...
function verifyJwt(token: string, secret: string): Record<string, unknown> | null {
  ...
  try {
    const payload = JSON.parse(base64urlDecode(payloadB64).toString("utf-8")) as Record<
      string,
      unknown
    >;
    return payload;
  ...
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (apiKeyEnv) {
      ...
      if (typeof providedKey === "string" && providedKey === apiKeyEnv) {
        ...
        (req as Request & { apiKey?: string }).apiKey = providedKey;
        next();
        return;
      }
    }
    ...
    if (jwtSecretEnv) {
      ...
      if (typeof authHeader === "string" && authHeader.startsWith("Bearer ")) {
        ...
        if (payload !== null) {
          (req as Request & { jwtPayload?: Record<string, unknown> }).jwtPayload =
            payload;
          next();
          return;
        }
