...
export class LegalDb {
  ...
  saveClauses(analysisId: string, clauses: ExtractedClause[]): void {
    ...
    const insertMany = this.db.transaction((rows: Array<Record<string, unknown>>) => {
  ...
  getAnalysis(id: string): DocumentAnalysis | undefined {
    const row = this.db
      ...
      .get(id) as DocumentAnalysis | undefined;
    return row;
  }
...
export async function createDb(dbPath: string): Promise<LegalDb> {
