...
export interface ClauseDiff {
  ...
  changed: Array<{ type: string; textA: string; textB: string }>;
 ...
 */
export async function toolCompareVersions(
  filePathA: string,
  filePathB: string,
  config: ServerConfig,
): Promise<string> {
  ...
  try {
    ...
    throw Object.assign(
      new Error(
        `Error reading file A: ${err instanceof Error ? err.message : String(err)}`,
      ),
      { mcpCode: code },
    );
  }
  ...
  try {
    ...
    throw Object.assign(
      new Error(
        `Error reading file B: ${err instanceof Error ? err.message : String(err)}`,
      ),
      { mcpCode: code },
    );
  }
  ...
  const added: ExtractedClause[] = [];
  const removed: ExtractedClause[] = [];
  const changed: Array<{ type: string; textA: string; textB: string }> = [];
  const unchanged: ExtractedClause[] = [];
  ...
  if (added.length > 0) {
    ...
    for (const clause of added) {
      lines.push(
        `#### + ${formatClauseType(clause.type)} (${Math.round(clause.confidence * 100)}% confidence)`,
      );
      lines.push("");
...
function buildTypeMap(clauses: ExtractedClause[]): Map<string, ExtractedClause> {
  const map = new Map<string, ExtractedClause>();
